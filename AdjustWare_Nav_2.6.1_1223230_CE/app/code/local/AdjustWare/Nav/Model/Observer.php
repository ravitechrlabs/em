...
class AdjustWare_Nav_Model_Observer
{
    ...
    public function onAdminhtmlEttributeEditPrepareForm($observer)
    {
        ...
        $form->addField('adjnav_categories', 'multiselect', array(
            'name'      => 'adjnav_categories[]',
            'label'     => Mage::helper('adminhtml')->__('Disable attribute on categories'),
            'title'     => Mage::helper('adminhtml')->__('Disable attribute on categories'),
            'values'    => Mage::getModel('adjnav/system_config_source_categories')->toOptionArray()
        ), 'adjnav_block_type');
    }

    public function onCatalogProductAttributeSave($observer)
    {
        $attribute = $observer->getAttribute();
        $categories = $attribute->getAdjnavCategories();
        if(!empty($categories) && is_array($categories))
        {
            $categories = implode(',',$categories);
            $attribute->setAdjnavCategories($categories);
        }
    }
}