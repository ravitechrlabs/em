...
class AdjustWare_Nav_Block_Catalog_Layer_View extends AdjustWare_Nav_Block_Rewrite_FrontCatalogLayerView
{
    ...
    public function getFilters()
    {
        if (is_null($this->_filterBlocks))
        {
            ...
            $filterableAttributes = $this->_getFilterableAttributes();
            $catId = Mage::getSingleton('catalog/session')->getAdjnavLastCategoryId();
            foreach ($filterableAttributes as $attribute) {
                $attrCatIds = explode(',', $attribute->getAdjnavCategories());
                if(!empty($catId) && in_array($catId, $attrCatIds))
                {
                    continue;
                }
